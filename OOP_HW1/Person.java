...
public class Person {
    ...
    public  OOP_HW1.status getStatus() {
        return status;
    }
    ...
    public static status setStatus (String status){
        ...
        for (String s : motherList) {
            ...
            if (Objects.equals(s, status)) {
                thisStatus = OOP_HW1.status.mother;
            }
        }
         for (String s: fatherList){
             s = s.toLowerCase();
            if (Objects.equals(s, status)){
                thisStatus = OOP_HW1.status.father;
            }
        }
         for (String s: daughterList) {
             s = s.toLowerCase();
             if (Objects.equals(s, status)) {
                 thisStatus = OOP_HW1.status.daughter;
             }
         }
         for (String s: sonList){
             s = s.toLowerCase();
             if (Objects.equals(s, status)){
                 thisStatus = OOP_HW1.status.son;
             }
         }
        return thisStatus;
    }
